...
if input_name == '#' or not input_name:
    input_name = "baki_puani, Yamac_Egimi, Sanat_Yapisi, Insaat_Alani, Kivrim_Faktor, Dolambac_Faktor, Kurp_Yogun, Yol_Deformasyon"
output_name = arcpy.GetParameterAsText(4)
if output_name == '#' or not output_name:
    output_name = "Alan_Uzun_Oran, Ekonomik"
...
Ekonomik_Etkinlik_Projection_Layer = arcpy.GetParameterAsText(8)
Ekonomik_Etkinlik_Target_Layer = arcpy.GetParameterAsText(9)
...
fields = arcpy.ListFields(Ekonomik_Etkinlik_Tablosu)
...
class DEA(object):
    ...
    def fit(self):
        self.__optimize()
        arcpy.AddMessage("Her Yol Biriminde Ekonomik Etkinlik Skorlari:\n")
        arcpy.AddMessage("---------------------------\n")
...
birlestirme_df.loc[sinir, 'Insaat_Alani'] = birlestirme_df.loc[sinir, 'Insaat_Alani'].apply(lambda x: max(6.6, x))
birlestirme_df.loc[sinir, 'Kivrim_Faktor'] = birlestirme_df.loc[sinir, 'Kivrim_Faktor'].apply(lambda x: max(1, x))
birlestirme_df.loc[sinir, 'Dolambac_Faktor'] = birlestirme_df.loc[sinir, 'Dolambac_Faktor'].apply(lambda x: max(1, x))
birlestirme_df.loc[sinir, 'Kurp_Yogun'] = birlestirme_df.loc[sinir, 'Kurp_Yogun'].apply(lambda x: max(3, x))
...
#Ekonomik Etkinlik Şablonları oluştur
if "\\" in Ekonomik_Etkinlik_Tablosu:
    arcpy.management.Copy(str(Ekonomik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekonomik_Etkinlik_Projeksiyon")
    arcpy.management.Copy(str(Ekonomik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekonomik_Etkinlik_Target")

else:
    arcpy.management.Copy(str(Workspace) + "\\" + str(Ekonomik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekonomik_Etkinlik_Projeksiyon")
    arcpy.management.Copy(str(Workspace) + "\\" + str(Ekonomik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekonomik_Etkinlik_Target")

Ekonomik_Etkinlik_Projection = str(Workspace) + r"\\Ekonomik_Etkinlik_Projeksiyon"
Ekonomik_Etkinlik_Target = str(Workspace) + r"\\Ekonomik_Etkinlik_Target"
fields = [f.name for f in arcpy.ListFields(str(Ekonomik_Etkinlik_Projection))]
fields.remove('Shape')
fields.remove('OBJECTID')
fields.remove(yol_name)
fields.remove('Shape_Length')
arcpy.DeleteField_management(str(Ekonomik_Etkinlik_Projection), fields)
arcpy.DeleteField_management(str(Ekonomik_Etkinlik_Target), fields)

arcpy.JoinField_management(Ekonomik_Etkinlik_Projection, yol_name, projection_table_path, yol_name)
arcpy.DeleteField_management(Ekonomik_Etkinlik_Projection, [yol_name + "_1"])
arcpy.JoinField_management(Ekonomik_Etkinlik_Target, yol_name, target_table_path, yol_name)
arcpy.DeleteField_management(Ekonomik_Etkinlik_Target, [yol_name + "_1"])
...
arcpy.Select_analysis(Ekonomik_Etkinlik_Target, Ekonomik_Etkinlik_Target_Etkin, query_1)

# "TARGET_Score" sütunu 1'den düşük olanları filtrele
query_2 = "TARGET_Score < 1"
Ekonomik_Etkinlik_Target_Etkin_Olmayan = str(Workspace) + r"\\Ekonomik_Etkin_Olmayan_Hedefler"
arcpy.Select_analysis(Ekonomik_Etkinlik_Target, Ekonomik_Etkinlik_Target_Etkin_Olmayan, query_2)


arcpy.Delete_management(table_path)
arcpy.Delete_management(projection_table_path)
arcpy.Delete_management(target_table_path)
arcpy.Delete_management(Ekonomik_Etkinlik_Target)
...
Ekonomik_Etkinlik_Projection_Layer = arcpy.mapping.Layer(Ekonomik_Etkinlik_Projection)
arcpy.mapping.AddLayer(df, Ekonomik_Etkinlik_Projection_Layer)
