...
if input_name == '#' or not input_name:
    input_name = "Yol_Uzunluk, Akarsu, Sanat_Yapisi, baki_puani, Yamac_Egimi, Yol_Egimi, Yol_Genisligi, Kazi_Yukseklik, Isletmeye_Acilan_Alan, Sev_Heyelan"
output_name = arcpy.GetParameterAsText(4)
if output_name == '#' or not output_name:
    output_name = "Doga_Koruma, Baglanti_Alan"
...
Ekolojik_Etkinlik_Projection_Layer = arcpy.GetParameterAsText(8)
Ekolojik_Etkinlik_Target_Layer = arcpy.GetParameterAsText(9)
...
fields = arcpy.ListFields(Ekolojik_Etkinlik_Tablosu)
...
class DEA(object):
    ...
    def fit(self):
        self.__optimize()
        arcpy.AddMessage("Her Yol Biriminde Ekolojik Etkinlik Skorlari:\n")
        arcpy.AddMessage("---------------------------\n")
...
birlestirme_df.loc[sinir, 'Kazi_Yukseklik'] = birlestirme_df.loc[sinir, 'Kazi_Yukseklik'].apply(lambda x: max(2, x))
birlestirme_df.loc[sinir, 'Yol_Genisligi'] = birlestirme_df.loc[sinir, 'Yol_Genisligi'].apply(lambda x: max(4, x))
birlestirme_df.loc[sinir, 'Sev_Heyelan'] = birlestirme_df.loc[sinir, 'Sev_Heyelan'].apply(lambda x: max(1, x))
...
#Ekolojik Etkinlik Şablonları oluştur
if "\\" in Ekolojik_Etkinlik_Tablosu:
    arcpy.management.Copy(str(Ekolojik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekolojik_Etkinlik_Projeksiyon")
    arcpy.management.Copy(str(Ekolojik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekolojik_Etkinlik_Target")

else:
    arcpy.management.Copy(str(Workspace) + "\\" + str(Ekolojik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekolojik_Etkinlik_Projeksiyon")
    arcpy.management.Copy(str(Workspace) + "\\" + str(Ekolojik_Etkinlik_Tablosu), str(Workspace) + r"\\Ekolojik_Etkinlik_Target")

Ekolojik_Etkinlik_Projection = str(Workspace) + r"\\Ekolojik_Etkinlik_Projeksiyon"
Ekolojik_Etkinlik_Target = str(Workspace) + r"\\Ekolojik_Etkinlik_Target"
fields = [f.name for f in arcpy.ListFields(str(Ekolojik_Etkinlik_Projection))]
fields.remove('Shape')
fields.remove('OBJECTID')
fields.remove(yol_name)
fields.remove('Shape_Length')
arcpy.DeleteField_management(str(Ekolojik_Etkinlik_Projection), fields)
arcpy.DeleteField_management(str(Ekolojik_Etkinlik_Target), fields)

arcpy.JoinField_management(Ekolojik_Etkinlik_Projection, yol_name, projection_table_path, yol_name)
arcpy.DeleteField_management(Ekolojik_Etkinlik_Projection, [yol_name + "_1"])
arcpy.JoinField_management(Ekolojik_Etkinlik_Target, yol_name, target_table_path, yol_name)
arcpy.DeleteField_management(Ekolojik_Etkinlik_Target, [yol_name + "_1"])
...
arcpy.Select_analysis(Ekolojik_Etkinlik_Target, Ekolojik_Etkinlik_Target_Etkin, query_1)

# "TARGET_Score" sütunu 1'den düşük olanları filtrele
query_2 = "TARGET_Score < 1"
Ekolojik_Etkinlik_Target_Etkin_Olmayan = str(Workspace) + r"\\Ekolojik_Etkin_Olmayan_Hedefler"
arcpy.Select_analysis(Ekolojik_Etkinlik_Target, Ekolojik_Etkinlik_Target_Etkin_Olmayan, query_2)


arcpy.Delete_management(table_path)
arcpy.Delete_management(projection_table_path)
arcpy.Delete_management(target_table_path)
arcpy.Delete_management(Ekolojik_Etkinlik_Target)
...
Ekolojik_Etkinlik_Projection_Layer = arcpy.mapping.Layer(Ekolojik_Etkinlik_Projection)
arcpy.mapping.AddLayer(df, Ekolojik_Etkinlik_Projection_Layer)
