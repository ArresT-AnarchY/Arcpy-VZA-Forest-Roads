...
if input_name == '#' or not input_name:
    input_name = "Yol_Egimi, Heyelan_Gocuk, baki_puani, Platform, Banket, Hendek, Agac_Yol_Yakin"
output_name = arcpy.GetParameterAsText(4)
if output_name == '#' or not output_name:
    output_name = "Karsilasma, Sanat_Yapisi, Baglanti_Alan, Hizmet, Isletmeye_Acilan_Alan, Emval_Eta"
...
Teknik_Etkinlik_Projection_Layer = arcpy.GetParameterAsText(8)
Teknik_Etkinlik_Target_Layer = arcpy.GetParameterAsText(9)
...
fields = arcpy.ListFields(Teknik_Etkinlik_Tablosu)
...
class DEA(object):
    ...
    def fit(self):
        self.__optimize()
        arcpy.AddMessage("Her Yol Biriminde Teknik Etkinlik Skorlari:\n")
        arcpy.AddMessage("---------------------------\n")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if all(dea.efficiency <= hedef_eff_siniri) and all(dea.efficiency > 0):
            break
...
birlestirme_df.loc[sinir, 'Platform'] = birlestirme_df.loc[sinir, 'Platform'].apply(lambda x: max(4, x))
birlestirme_df.loc[sinir, 'Banket'] = birlestirme_df.loc[sinir, 'Banket'].apply(lambda x: max(0.5, x))
birlestirme_df.loc[sinir, 'Hendek'] = birlestirme_df.loc[sinir, 'Hendek'].apply(lambda x: max(0.5, x))
...
#Teknik Etkinlik Şablonları oluştur
if "\\" in Teknik_Etkinlik_Tablosu:
    arcpy.management.Copy(str(Teknik_Etkinlik_Tablosu), str(Workspace) + r"\\Teknik_Etkinlik_Projeksiyon")
    arcpy.management.Copy(str(Teknik_Etkinlik_Tablosu), str(Workspace) + r"\\Teknik_Etkinlik_Target")

else:
    arcpy.management.Copy(str(Workspace) + "\\" + str(Teknik_Etkinlik_Tablosu), str(Workspace) + r"\\Teknik_Etkinlik_Projeksiyon")
    arcpy.management.Copy(str(Workspace) + "\\" + str(Teknik_Etkinlik_Tablosu), str(Workspace) + r"\\Teknik_Etkinlik_Target")

Teknik_Etkinlik_Projection = str(Workspace) + r"\\Teknik_Etkinlik_Projeksiyon"
Teknik_Etkinlik_Target = str(Workspace) + r"\\Teknik_Etkinlik_Target"
fields = [f.name for f in arcpy.ListFields(str(Teknik_Etkinlik_Projection))]
fields.remove('Shape')
fields.remove('OBJECTID')
fields.remove(yol_name)
fields.remove('Shape_Length')
arcpy.DeleteField_management(str(Teknik_Etkinlik_Projection), fields)
arcpy.DeleteField_management(str(Teknik_Etkinlik_Target), fields)

arcpy.JoinField_management(Teknik_Etkinlik_Projection, yol_name, projection_table_path, yol_name)
arcpy.DeleteField_management(Teknik_Etkinlik_Projection, [yol_name + "_1"])
arcpy.JoinField_management(Teknik_Etkinlik_Target, yol_name, target_table_path, yol_name)
arcpy.DeleteField_management(Teknik_Etkinlik_Target, [yol_name + "_1"])
...
arcpy.Select_analysis(Teknik_Etkinlik_Target, Teknik_Etkinlik_Target_Etkin, query_1)

# "TARGET_Score" sütunu 1'den düşük olanları filtrele
query_2 = "TARGET_Score < 1"
Teknik_Etkinlik_Target_Etkin_Olmayan = str(Workspace) + r"\\Teknik_Etkin_Olmayan_Hedefler"
arcpy.Select_analysis(Teknik_Etkinlik_Target, Teknik_Etkinlik_Target_Etkin_Olmayan, query_2)


arcpy.Delete_management(table_path)
arcpy.Delete_management(projection_table_path)
arcpy.Delete_management(target_table_path)
arcpy.Delete_management(Teknik_Etkinlik_Target)
...
Teknik_Etkinlik_Projection_Layer = arcpy.mapping.Layer(Teknik_Etkinlik_Projection)
arcpy.mapping.AddLayer(df, Teknik_Etkinlik_Projection_Layer)
