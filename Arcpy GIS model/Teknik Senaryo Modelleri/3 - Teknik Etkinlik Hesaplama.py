...
##########################################################################################################
# Platform Genişliğini 5 metre istediğimiz için özel bir normalize gerektirmektedir.
##########################################################################################################
# Sütun adlarını al
...
# Pandas DataFrame
platform_pd = pd.DataFrame(data, columns=fields)
...
##########################################################################################################
# Hendek Genişliğini 0,5 metre istediğimiz için özel bir normalize gerektirmektedir.
##########################################################################################################
# Sütun adlarını al
fields = [field.name for field in arcpy.ListFields(normalize_tablo)]
...
# Pandas DataFrame
banket_pd = pd.DataFrame(data, columns=fields)
...
##########################################################################################################
# Hendek Genişliğini 1 metre istediğimiz için özel bir normalize gerektirmektedir.
##########################################################################################################
# Sütun adlarını al
fields = [field.name for field in arcpy.ListFields(normalize_tablo)]
...
# Pandas DataFrame
hendek_pd = pd.DataFrame(data, columns=fields)
...
##########################################################################################################
# Platform Genişliğini 5 metre olarak istemiştik bu nedenle analize giren data ve hedef değerleri için
# ayrı ayrı yeniden dönüşüm yapılmalıdır.
##########################################################################################################
# Data Platform Genişliği Denormalize..
...
uzaklik5 = [abs(platformaralik - 5) for platformaralik in platform_degeri]

denormalize_scor = []
for platformaralik, value in zip(platform_degeri, uzaklik5):
    if platformaralik < 5:
        denormalize_scor.append(5 - value)
    else:
        denormalize_scor.append(value + 5)

data_denormalize["Platform"] = denormalize_scor

# Hedef Platform Genişliği Denormalize..
denormalize_scor1 = []
for platformaralik, value in zip(platform_degeri, uzaklik5):
    if platformaralik < 5:
        denormalize_scor1.append(5 - value)
    else:
        denormalize_scor1.append(value + 5)

hedef_denormalize["Platform"] = denormalize_scor1

##########################################################################################################
# Banket Genişliğini 0.5 metre olarak istemiştik bu nedenle analize giren data ve hedef değerleri için
# ayrı ayrı yeniden dönüşüm yapılmalıdır.
##########################################################################################################
# Data Banket Genişliği Denormalize..
fields = [field.name for field in arcpy.ListFields(girdi_cikti_verileri)]
...
uzaklik05 = [abs(banketaralik - 0.5) for banketaralik in banket_degeri]

denormalize_scor2 = []
for banketaralik, value in zip(banket_degeri, uzaklik05):
    if banketaralik < 0.5:
        denormalize_scor2.append(0.5 - value)
    else:
        denormalize_scor2.append(value + 0.5)

data_denormalize["Banket"] = denormalize_scor2

# Hedef Banket Genişliği Denormalize..
denormalize_scor3 = []
for banketaralik, value in zip(banket_degeri, uzaklik05):
    if banketaralik < 0.5:
        denormalize_scor3.append(0.5 - value)
    else:
        denormalize_scor3.append(value + 0.5)

hedef_denormalize["Banket"] = denormalize_scor3

##########################################################################################################
# Hendek Genişliğini 1 metre olarak istemiştik bu nedenle analize giren data ve hedef değerleri için
# ayrı ayrı yeniden dönüşüm yapılmalıdır.
##########################################################################################################
# Hendek Banket Genişliği Denormalize..
fields = [field.name for field in arcpy.ListFields(girdi_cikti_verileri)]
...
uzaklik1 = [abs(hendekaralik - 1) for hendekaralik in hendek_degeri]

denormalize_scor4 = []
for hendekaralik, value in zip(hendek_degeri, uzaklik1):
    if hendekaralik < 1:
        denormalize_scor4.append(1 - value)
    else:
        denormalize_scor4.append(value + 1)

data_denormalize["Hendek"] = denormalize_scor4

# Hedef Hendek Genişliği Denormalize..
denormalize_scor5 = []
for hendekaralik, value in zip(hendek_degeri, uzaklik1):
    if hendekaralik < 1:
        denormalize_scor5.append(1 - value)
    else:
        denormalize_scor5.append(value + 1)

hedef_denormalize["Hendek"] = denormalize_scor5
...
for yolaralik, value in zip(yol_aralik_degeri, uzaklik500):
    if yolaralik < 500:
        denormalize_scor6.append(500 - value)
    else:
        denormalize_scor6.append(value + 500)
...
for yolaralik, value in zip(yol_aralik_degeri, uzaklik500):
    if yolaralik < 500:
        denormalize_scor7.append(500 - value)
    else:
        denormalize_scor7.append(value + 500)

hedef_denormalize["Yol_Aralik"] = denormalize_scor7
...
arcpy.AddWarning("Teknik normalizasyon degeri           - Min= 0 - Max= 10      - birim= (ha/km)")
...
# Etkin olanlar, ALL tablosundan oluşan, Teknik Etkinlik Analizi içinde "Score" sütunu 1'e eşit olanları filtrelemeli
...
# Etkin olanlar, ALL tablosundan oluşan, Teknik Etkinlik Analizi içinde "Score" sütunu 1'den küçük olanları filtrelemeli
query_2 = "Score < 1"
